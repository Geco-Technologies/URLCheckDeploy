...
if (window.checkExtensionLoaded) {
  ...
  // ── Generic credential-harvesting detection ───────────────────────────────
  //
  ...
  function scoreGenericPhishingUrl(url) {
    ...
    try {
      ...
      // Multiple hyphens in hostname — often constructed subdomains on
      // compromised/cheap domains used to look "official"
      const hyphenCount = (hostname.match(/-/g) || []).length;
      if (hyphenCount >= 3) {
        score += 15;
        reasons.push(`${hyphenCount} hyphens in hostname (constructed subdomain pattern)`);
  ...
  /**
   * Detect credential-harvesting form elements on the current page.
   * Returns { hasPasswordField, hasEmailField, formPostsExternally }
   */
  function detectCredentialForm() {
    try {
      const passwordFields = document.querySelectorAll('input[type="password"]');
      if (passwordFields.length === 0) return { hasPasswordField: false, hasEmailField: false, formPostsExternally: false };

      const emailFields = document.querySelectorAll(
        'input[type="email"], input[type="text"][name*="email"], input[type="text"][name*="user"], input[type="text"][placeholder*="email" i], input[type="text"][placeholder*="user" i]'
      );

      // Check if any form containing a password field posts to a different domain
      let formPostsExternally = false;
      for (const pf of passwordFields) {
        const form = pf.closest('form');
        if (form) {
          const action = form.getAttribute('action') || '';
          if (action && action.startsWith('http') && !action.startsWith(location.origin)) {
            formPostsExternally = true;
          }
        }
      }
      ...
      return { hasPasswordField: false, hasEmailField: false, formPostsExternally: false };
    }
  }
  ...
  function checkGenericCredentialHarvesting() {
    ...
    if (blocked || warned) {
      const trigger = blocked
        ? (score >= 90 && !hasForm ? 'URL-only block' : 'form+URL block')
        : 'warning';
      logger.warn(
        `🎣 GENERIC CREDENTIAL HARVESTING [${trigger}]: ` +
        `score=${totalScore} (url=${score} form=${totalScore - score})`
      );
      reasons.forEach(r => logger.warn(`  • ${r}`));
      if (form.hasPasswordField)    logger.warn('  • Password input field present');
      if (form.hasEmailField)       logger.warn('  • Email/username input field present');
      if (form.formPostsExternally) logger.warn('  • Form posts credentials to external domain');
    }

    return { blocked, warned, score: totalScore, urlScore: score, reasons, form };
  }

  // ── End generic credential-harvesting detection ───────────────────────────

  /**
   * Check if URL matches any external allowlist pattern (from ExternalFeedsManager via session storage)
   */
  function checkExternalAllowlist(url) {
    try {
      const patterns = window.checkExternalFeeds?.allowlistPatterns;
      ...
      for (const raw of patterns) {
        ...
        try {
          ...
          if (new RegExp(pattern, 'i').test(url)) {
            logger.log(`✅ URL matched external allowlist pattern "${raw}": ${url}`);
            return true;
          }
        } catch (_) {
      ...
      logger.warn('Error checking external allowlist:', err.message);
      return false;
    }
  }

  /**
   * Check if URL matches any external blocklist entry (from ExternalFeedsManager via session storage)
   */
  function checkExternalBlocklist(url) {
    try {
      const entries = window.checkExternalFeeds?.blocklistEntries;
      ...
      for (const raw of entries) {
        ...
        try {
          const pattern = raw.includes('*')
            ...
            : raw;
          if (new RegExp(pattern, 'i').test(url)) {
            logger.warn(`🚫 URL matched external blocklist entry "${raw}": ${url}`);
            return true;
          }
        } catch (_) {
          if (url.includes(raw)) return true;
      ...
      logger.warn('Error checking external blocklist:', err.message);
      return false;
    }
  }
  ...
  async function runProtection(isRerun = false, forceRescan = false, options = {}) {
    ...
    try {
      ...
      // Load external feeds from session storage (written by ExternalFeedsManager in background)
      try {
        const feedsData = await new Promise((resolve) => {
          chrome.storage.session.get(['external_feeds'], (r) => resolve(r?.external_feeds || null));
        });
        window.checkExternalFeeds = feedsData;
        logger.log(
          `[Feeds] session storage: ${
            feedsData
              ? `${feedsData.blocklistEntries?.length ?? 0} block, ${feedsData.allowlistPatterns?.length ?? 0} allow`
              : 'null (not yet populated)'
          }`
        );
      } catch (_) {
        window.checkExternalFeeds = null;
      }

      // Early exit if URL is in external allowlist feed
      if (checkExternalAllowlist(window.location.href)) {
        logger.log(`✅ URL ALLOWLISTED BY EXTERNAL FEED - skipping scan: ${window.location.href}`);
        return;
      }

      // Early exit if URL is in external blocklist feed — hard block regardless of page content
      if (checkExternalBlocklist(window.location.href)) {
        logger.warn(`🚫 URL BLOCKED BY EXTERNAL FEED: ${window.location.href}`);
        const blockUrl = chrome.runtime.getURL("blocked.html") +
          "?url=" + encodeURIComponent(window.location.href) +
          "&reason=" + encodeURIComponent("URL matches an administrator-configured blocklist feed") +
          "&type=external_blocklist";
        try {
          await chrome.runtime.sendMessage({ type: "REDIRECT_TO_BLOCKED_PAGE", url: blockUrl });
        } catch (_) {
      ...
      if (!msDetection.isLogonPage) {
        ...
        if (!msDetection.hasElements) {
          ...
          // ── Generic credential harvesting check ─────────────────────────
          // Catches phishing pages with no Microsoft branding by combining
          // suspicious URL structure + credential input form detection.
          const genericCheck = checkGenericCredentialHarvesting();

          if (genericCheck.blocked && config.enablePageBlocking !== false) {
            logger.warn(`🚫 GENERIC PHISHING BLOCKED (score: ${genericCheck.score})`);
            logProtectionEvent({
              type: 'threat_detected',
              action: 'blocked',
              url: location.href,
              origin: currentOrigin,
              reason: `Generic credential harvesting page: ${genericCheck.reasons.slice(0, 3).join('; ')}`,
              severity: 'high',
            });
            const blockUrl = chrome.runtime.getURL('blocked.html') +
              '?url='    + encodeURIComponent(location.href) +
              '&reason=' + encodeURIComponent('This page appears to be harvesting credentials. ' + genericCheck.reasons.slice(0, 2).join('. ')) +
          ...
          if (genericCheck.warned) {
            logger.warn(`⚠️ GENERIC PHISHING WARNING (score: ${genericCheck.score})`);
            logProtectionEvent({
              type: 'threat_detected',
              action: 'warned',
              url: location.href,
              origin: currentOrigin,
              reason: `Suspicious credential page: ${genericCheck.reasons.slice(0, 3).join('; ')}`,
              severity: 'medium',
            });
            // Show a warning banner — reuse the existing notification system
            try {
              chrome.runtime.sendMessage({
                type: 'SHOW_PHISHING_WARNING',
                url: location.href,
                reason: 'This page appears to be collecting credentials on a suspicious domain.',
                score: genericCheck.score,
              });
            } catch (_) {}
          }
          // ── End generic credential harvesting check ──────────────────────
  ...
  function showWarningBanner(reason, analysisData) {
    try {
      ...
      const applyBranding = (bannerEl, branding) => {
        ...
        try {
          const companyName =
            branding.companyName || branding.productName || "Unite Networks";
